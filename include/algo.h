 ...
 */
template <typename T> auto Max(const T &a, const T &b) noexcept -> const T & {
  return a > b ? a : b;
}
...
auto Copy(InputIterator first, InputIterator last,
          OutputIterator result) noexcept -> OutputIterator {
  while (first != last) {
    *result = *first;
    ++result;
    ++first;
...
auto Fill(ForwardIterator first, ForwardIterator last, const T &value) noexcept
    -> void {
  while (first != last) {
    *first = value;
    ++first;
 ...
 * @param b Second value.
 */
template <typename T> auto Swap(T &a, T &b) noexcept -> void {
  T temp = a;
  a = b;
  b = temp;
}
