...
namespace easystl {
    ...
    * @brief A memory allocator that use `malloc`, `realloc`, and `free` for memory management.
    * This allocator allows fow the ise of a custom out-of-memory(OOM) handler.
    */
    class MallocAllocator {
    public:
        /**
         ...
         * @return A pointer to the allocated memory block, or invokes an OOM handler if `malloc` fails.
         */
        static auto Allocate(const std::size_t size) -> void * {
            void *result = malloc(size);
            if (result == nullptr) { result = MallocInOom(size); } // Call OOM handler if allocation fails
            return result;
        }
        ...
        * @param{unnamed} Unused parameter, present for compatibility with other allocators.
        */
        ...
        * @return A pointer to the reallocated memory block, or invokes an OOM handler if `realloc` fails.
        */
        static auto Reallocate(void *obj, std::size_t /*oldsize*/, const size_t newsize) -> void * {
            void *result = realloc(obj, newsize);
            if (result == nullptr) { result = ReallocInOom(obj, newsize); } // Call OOM handler if reallocation fails.
            return result;
        }

        /**
        * @brief Sets a custom out-of-memory (OOM) handler to be invoked when allocation fails.
    ...
    private:
        ...
        * @return A pointer to the allocated memory block, or aborts if OOM handler cannot resolve the issue.
        */
        ...
        * @return A pointer to the reallocated memory block, or aborts if OOM handler cannot resolve the issue.
        */
    ...
    */
    ...
    /**
    * @brief Attempts to allocate memory when `malloc` fails, invoking the OOM handler.
    * If the handler is set, it will retry 3 times before returning `nullptr` or aborting.
    * @param size The size of memory to allocate.
    * @return A pointer to the allocated memory, or `nullptr` if allocation fails after retries.
    */
    inline auto MallocAllocator::MallocInOom(const std::size_t size) -> void * {
        ...
        while (retry_count < max_retries) {
            ...
            if (void *result = malloc(size)) { return result; } // Try allocating memory again
            retry_count++;
        }
        return nullptr;
    }
    ...
    /**
    * @brief Attempts to reallocate memory when `realloc` fails, invoking the OOM handler.
    * If the handler is set, it will retry up to 3 times before returning `nullptr` or aborting.
    * @param obj Pointer to the existing memory block to reallocate.
    * @param size The new size of the memory block.
    * @return A pointer to the reallocated memory, or `nullptr` if reallocation fails after retries.
    */
