...
namespace easystl {
    ...
    auto uninitialized_copy(InputIter first, InputIter last, ForwardIter result) -> ForwardIter {
        auto current = result;
        try {
    ...
    */
    ...
    auto uninitialized_fill(ForwardIter first, ForwardIter last, const T &value) -> void {
        auto current = first;
        try {
    ...
    /**
    * @brief Fills uninitialized memory with a specified value for a given number of elements.
    *
    * This function constructs objects with the specified value in the range
    ...
    */
    ...
    auto uninitialized_fill_n(ForwardIter first, Size n, const T &value) -> ForwardIter {
        auto current = first;
        try {
            for (; n > 0; --n, ++current) {
                Construct(&*current, value);
